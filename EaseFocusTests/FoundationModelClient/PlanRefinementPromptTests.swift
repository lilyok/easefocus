...
struct PlanRefinementPromptTests {
    ...
    func sendsACompletionSummaryAndStableTaskIDsWithoutSessionDetails() {
        ...
        #expect(instructions.contains("do not add a duplicate"))
        #expect(message.contains("If a requested task already exists"))
        #expect(message.contains("Put extra context into details"))
